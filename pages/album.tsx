...
export const getServerSideProps = async () => {
    ...
    const Images = await Promise.all(
        ImageFolderArray.map(async (folder: string) => {
            ...
            const convertedData = data
                ...
                .filter(
                    (row: any) => row !== `${folder}/.emptyFolderPlaceholder`
                );
    ...
    return {
        props: { body: { convertedTodoResponse, Images, profile2 } },
    };
};
...
const AlbumPage = ({ body }: any) => {
    ...
    // console.log("ログインユーザー名：" + body.profile2[0].username);
    ...
    const onClickRemove = (fileName: string) => {
        setImportingFiles(
            importingFile.filter((file) => file.name !== fileName)
        );
    };
                ...
                <Box m={10}>
                    ...
                    <Box ml={"auto"} mr={"auto"}>
                        ...
                        <Select
                            value={selectedValue}
                            onChange={(e) =>
                                setSelectedValue(e.target.value as TodoName)
                            }
                                            ...
                                            <AspectRatio ratio={1}>
                                                <Image
                                                    src={
                                                        ...
                                                        process.env
                                                            .NEXT_PUBLIC_SUPABASE_SUBDOMAIN! +
                                                        process.env
