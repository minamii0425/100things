...
import type { Methods as Methods2 } from './profiles'
import type { Methods as Methods3 } from './profiles/_id'
import type { Methods as Methods4 } from './tags'
import type { Methods as Methods5 } from './tags/_id'
import type { Methods as Methods6 } from './todos'
import type { Methods as Methods7 } from './todos/_id'
import type { Methods as Methods8 } from './todos_comments'
import type { Methods as Methods9 } from './todos_comments/_id'
import type { Methods as Methods10 } from './todos_tags'
import type { Methods as Methods11 } from './todos_tags/_id'

const api = <T>({ baseURL, fetch }: AspidaClient<T>) => {
  const prefix = (baseURL === undefined ? 'http://localhost:3011/api/' : baseURL).replace(/\/$/, '')
  const PATH0 = '/comments'
  const PATH1 = '/profiles'
  const PATH2 = '/tags'
  const PATH3 = '/todos'
  const PATH4 = '/todos_comments'
  const PATH5 = '/todos_tags'
  const GET = 'GET'
  const POST = 'POST'
  const PUT = 'PUT'
  const DELETE = 'DELETE'
    ...
    todos_tags: {
      _id: (val1: number | string) => {
        ...
        return {
           ...
           * @returns Successful operation
           */
          delete: (option: { query: Methods11['delete']['query'], headers?: Methods11['delete']['reqHeaders'] | undefined, config?: T | undefined }) =>
           ...
           * @returns Successful
           */
          put: (option: { body: Methods11['put']['reqBody'], query: Methods11['put']['query'], headers?: Methods11['put']['reqHeaders'] | undefined, config?: T | undefined }) =>
          ...
          $path: (option?: { method: 'delete'; query: Methods11['delete']['query'] } | { method: 'put'; query: Methods11['put']['query'] } | undefined) =>
            `${prefix}${prefix1}${option && option.query ? `?${dataToURLString(option.query)}` : ''}`
        }
      },
      /**
      ...
      post: (option: { body: Methods10['post']['reqBody'], headers?: Methods10['post']['reqHeaders'] | undefined, config?: T | undefined }) =>
        fetch<Methods10['post']['resBody'], BasicHeaders, Methods10['post']['status']>(prefix, PATH5, POST, option).json(),
      /**
       * Todo-タグの登録を行う。
       * @returns Successful operation
       */
      $post: (option: { body: Methods10['post']['reqBody'], headers?: Methods10['post']['reqHeaders'] | undefined, config?: T | undefined }) =>
        fetch<Methods10['post']['resBody'], BasicHeaders, Methods10['post']['status']>(prefix, PATH5, POST, option).json().then(r => r.body),
      /**
       * Todo-タグの複数削除を行う。
       * @returns Successful operation
       */
      delete: (option: { body: Methods10['delete']['reqBody'], headers?: Methods10['delete']['reqHeaders'] | undefined, config?: T | undefined }) =>
        fetch<Methods10['delete']['resBody'], BasicHeaders, Methods10['delete']['status']>(prefix, PATH5, DELETE, option).json(),
      /**
       * Todo-タグの複数削除を行う。
       * @returns Successful operation
       */
      $delete: (option: { body: Methods10['delete']['reqBody'], headers?: Methods10['delete']['reqHeaders'] | undefined, config?: T | undefined }) =>
        fetch<Methods10['delete']['resBody'], BasicHeaders, Methods10['delete']['status']>(prefix, PATH5, DELETE, option).json().then(r => r.body),
      $path: () => `${prefix}${PATH5}`
    }
  }
}
